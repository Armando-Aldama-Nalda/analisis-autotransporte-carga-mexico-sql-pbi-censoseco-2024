...
El autotransporte de carga es uno de los sectores clave para el crecimiento sostenido de la economía mexicana, representando aproximadamente 4% del PIB nacional de acuerdo con datos de 2025. Sin embargo, es un sector fragmentado donde conviven desde micro-empresas familiares hasta grandes consorcios logísticos.
...
-Para este análisis, construí un ecosistema de datos que garantiza limpieza y escalabilidad:
  ...
  -Ademas de comparar empresas por número de empleados, las clasifiqué por el valor de su producción anual. Usé Window Functions (SUM() OVER) para dividir el sector en 5 grupos iguales (quintiles).

III. Desafíos Técnicos y Soluciones. 

Limpieza de Datos (Data Wrangling). Los datos crudos del censo venían con estructuras jerárquicas y filas de totales que dificultaban el análisis. Por ello, implementé Vistas en SQL para:
...
-Oportunidades de Inversión: Identifiqué que los Quintiles 3 y 4 son el "motor silencioso": generan una producción sólida (20-25 MDP anuales) y muestran una alta tasa de reinversión.
